...
ll prod(vector<ll>& a, ll t){
	ll sum = 0;
	for(auto& e : a) { sum += (t/e); sum = min(sum, 1000000001LL);}
	return sum;
}
...
void solve()
{
	...
	for(auto& e : a)
		cin >> e;
	ll l = 1, r = LONG_LONG_MAX, mid, p1, p2;
	while(l<r){
		mid = l+(r-l)/2;
		p1 = prod(a, mid);
		p2 = prod(a, mid-1);
		if(p2 >= p) { r = mid-1; continue; }
		if(p1<p) {l = mid + 1; continue; }
		if(p1>=p and p2 < p) { cout << mid; return; }
	}
	cout << l;
}
