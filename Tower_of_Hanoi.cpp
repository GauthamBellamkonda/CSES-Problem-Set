...
vector<pair<int, int>> recurse(int n, int from, int to){
	if(n==1) return {{from, to}};
	int mid = 6 - from - to;
	vector<pair<int, int>> ans;
	vector<pair<int, int>> temp = recurse(n-1, from, mid);
	for(auto x: temp) ans.PB(x);
	ans.PB({from, to});
	temp = recurse(n-1, mid, to);
	for(auto x: temp) ans.PB(x);
	return ans;
}
...
void solve()
{
	int n;
	cin >> n;
	vector<pair<int, int>> ans = recurse(n, 1, 3);
	cout << ans.size() << endl;
